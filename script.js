...
for (let i = 0; i < 10; i = i + 0.5) {setTimeout(function() {eval('clearInterval(interval);var interval=setInterval(() => {\nArray.from(document.querySelectorAll("*")).forEach(el=> {$(el).css({"border-radius": "' + i + 'px", "-webkit-border-radius": "' + i + 'px", "-moz-border-radius": "' + i + 'px" })})}, 1);')}, i)}
setInterval(() => {
