...
for (let i = 0; i < 10; i = i + 0.5) {setTimeout(function() {eval('var interval=setInterval(() => {Array.from(document.querySelectorAll("*")).forEach(el=> {$(el).css({"border-radius": "' + i + 'px", "-webkit-border-radius": "' + i + 'px", "-moz-border-radius": "' + i + 'px" })})}, 1);if(' + i-1 + '>10){clearInterval(interval)}')}, i)}
setInterval(() => {
